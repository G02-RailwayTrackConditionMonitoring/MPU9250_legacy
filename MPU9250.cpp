...
void MPU9250::readMagData(int16_t *destination)
{
  ...
  if (readAK8963Registers(AK8963_ST1, 1, u) & 0x01)
  {
    // Read the six raw data and ST2 registers sequentially into data array
    readAK8963Registers(AK8963_XOUT_L, 7, &rawData[0]);
...
void MPU9250::initMPU9250()
{
  ...
  //*******************************************
  // *** Magneto ***
  writeByte(_I2Caddr, USER_CTRL, 0X20);
  writeByte(_I2Caddr, I2C_MST_CTRL, 0x0D);
  writeAK8963Register(0x0A, 0x00);       // set AK8963 to Power Down
  writeByte(_I2Caddr, PWR_MGMT_1, 0x80); // reset the MPU9250
  delay(1);                              // wait for MPU-9250 to come back up
  writeAK8963Register(0x0B, 0x01);       // reset the AK8963
  writeByte(_I2Caddr, USER_CTRL, 0X20);
  writeByte(_I2Caddr, I2C_MST_CTRL, 0x0D);
  writeAK8963Register(AK8963_CNTL, 0x16);
  Serial.println(writeAK8963Register(AK8963_CNTL, 0x0F)); // Enter Fuse ROM access mode
  delay(100);
  ...
  asa[0] = (float)(rawData[0] - 128) / 256. + 1.;
  asa[1] = (float)(rawData[1] - 128) / 256. + 1.;
  asa[2] = (float)(rawData[2] - 128) / 256. + 1.;
...
*/
  writeByte(_I2Caddr, PWR_MGMT_1, 0x01);
  delay(200);
  writeByte(_I2Caddr, PWR_MGMT_2, 0x07);
  delay(100);
  writeByte(_I2Caddr, CONFIG, 0x00);
  delay(100);
  writeByte(_I2Caddr, SMPLRT_DIV, 0x00);
  delay(100);
  writeByte(_I2Caddr, SMPLRT_DIV, 0x00);
  delay(100);
  uint8_t accel_config = readByte(_I2Caddr, ACCEL_CONFIG);
  accel_config = accel_config | 0x18;
  writeByte(_I2Caddr, ACCEL_CONFIG, accel_config);
  delay(100);
  uint8_t accel_config_2 = readByte(_I2Caddr, ACCEL_CONFIG2);
  accel_config_2 = accel_config_2 & 0xF0;
  accel_config_2 = accel_config_2 | 0x18;
  writeByte(_I2Caddr, ACCEL_CONFIG2, accel_config_2);
  delay(100);
  ...
  float destination[3];
  // TODO: Test this!! Likely doesn't work
  Serial.println(writeAK8963Register(AK8963_CNTL, 0x00)); // Power down magnetometer
  delay(100);
  Serial.println(writeAK8963Register(AK8963_CNTL, 0x0F)); // Enter Fuse ROM access mode
  delay(100);
  ...
  // Read the x-, y-, and z-axis calibration values
  readAK8963Registers(AK8963_ASAX, 3, &rawData[0]);
  Serial.println(String(rawData[0]));
  Serial.println(String(rawData[1]));
  Serial.println(String(rawData[2]));
  ...
  Serial.println(writeAK8963Register(AK8963_CNTL, 0x16));
  delay(100);
  */
  /*
    setupMagForSPI();
...
void MPU9250::calibrateMPU9250(float *accelBias)
{
...
int MPU9250::writeAK8963Register(uint8_t subAddress, uint8_t data)
{
  ...
  if (writeByte(_I2Caddr, I2C_SLV0_ADDR, AK8963_ASTC) < 0)
  {
    return -1;
  }
  // set the register to the desired AK8963 sub address
  if (writeByte(_I2Caddr, I2C_SLV0_REG, subAddress) < 0)
  {
    return -2;
  }
  // store the data for write
  if (writeByte(_I2Caddr, I2C_SLV0_DO, data) < 0)
  {
    return -3;
  }
  // enable I2C and send 1 byte
  if (writeByte(_I2Caddr, I2C_SLV0_CTRL, 0x81) < 0)
  {
    return -4;
  }
  // read the register and confirm
  uint8_t b;
  if (readAK8963Registers(subAddress, 1, b) < 0)
  {
    return -5;
  }
  if (b == data)
...
int MPU9250::readAK8963Registers(uint8_t subAddress, uint8_t count, uint8_t *dest)
{
  ...
  if (writeByte(_I2Caddr, I2C_SLV0_ADDR, AK8963_ASTC | READ_FLAG) < 0)
  {
    return -1;
  }
  // set the register to the desired AK8963 sub address
  ...
  if (writeByte(_I2Caddr, I2C_SLV0_CTRL, READ_FLAG | count) < 0)
  {
    return -3;
  }
  delay(1);
  int _status = readBytes(_I2Caddr, EXT_SENS_DATA_00, count, dest);
